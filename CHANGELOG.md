...
### Added
- New `api_arn` output for stable IAM policy construction
- New `graphql_field_arn_prefix` output to simplify resolver field permission ARNs

### Changed
- README usage updated to `~> 1.1`
- IAM policy example now uses `graphql_field_arn_prefix` instead of URL string replacement
