...
resource "aws_appsync_graphql_api" "dr" {
  ...
  dynamic "log_config" {
    for_each = var.enable_logging ? [1] : []
    content {
      cloudwatch_logs_role_arn = aws_iam_role.appsync_logging_dr[0].arn
      field_log_level          = var.log_level
    }
  }
...
resource "aws_cloudwatch_log_group" "appsync_dr" {
  count    = var.enable_logging && local.dr_enabled ? 1 : 0
  provider = aws.dr

  name              = "/aws/appsync/apis/${aws_appsync_graphql_api.dr[0].id}"
  retention_in_days = 30
  tags              = var.tags
}
...
resource "aws_iam_role" "appsync_logging_dr" {
  count = var.enable_logging && local.dr_enabled ? 1 : 0
  name  = "${var.name}-appsync-logging-dr"
...
resource "aws_iam_role_policy" "appsync_logging_dr" {
  count = var.enable_logging && local.dr_enabled ? 1 : 0
  name  = "${var.name}-appsync-logging-dr"
  role  = aws_iam_role.appsync_logging_dr[0].id
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = "${aws_cloudwatch_log_group.appsync_dr[0].arn}:*"
    }]
  })
}
...
resource "aws_appsync_api_key" "dr" {
  count    = var.enable_api_key && local.dr_enabled ? 1 : 0
  provider = aws.dr
...
resource "aws_appsync_datasource" "none_dr" {
  count    = local.dr_enabled ? 1 : 0
  provider = aws.dr
